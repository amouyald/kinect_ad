...
void OverlayImage(IplImage* src, IplImage* overlay, CvPoint location, CvScalar S, CvScalar D, int size_width, int size_length)
{
...
int main(int argc, char **argv) {
	...
	bool Nobody = TRUE, Arrived = FALSE, Playing = FALSE, Playing_finished = FALSE;
	int Wait = 4;
	...
	//Loading images
	IplImage *bubble = cvLoadImage("./bubble.png");
	IplImage *background = cvLoadImage("./background.png");
	...
	CvScalar S_bubble = cvScalar(1, 1, 1, 1);
	CvScalar D_bubble = cvScalar(0.6, 0.6, 0.6, 0.6);
	
	//CvScalar S_Punch = cv Scalar(0.5, 0.5, 0.5, 0.5);
	//CvScalar D_Punch = cvScalar(0.5, 0.5, 0.5, 0.5);
	...
	while (!die) {
		...
		cvFlip(img_rgb, NULL, 1);
		OverlayImage(img_rgb, background, pt1 , S,D, 640,480);
		
//		cout << "DEPTH OF BLOB : ";
//		cout << (int)depthf.data[y_avg*640+x_avg];
//		cout << "\n";
		
		if (depthf.data[y_avg*640+x_avg] < 80 && Nobody && Wait==0) {
		...
		if (Arrived) 
			cv::putText(rgbMat, "Pop a bubble !", Point(60,200), FONT_HERSHEY_SIMPLEX, 2,  cv::Scalar(250,0,0), 6, 6, false);
		...
		for (int k = 0; k<8; k++) {
			...
			p.x=x_circle[k];
			p.y=y_circle[k];
			OverlayImage(img_rgb, bubble,p, S_bubble,D_bubble,60,60);
			...
			if ( !Nobody){
				...
				if (Playing){
					if (timer > 26) {
						cv::putText(rgbMat, "Pop as many", Point(90,200), FONT_HERSHEY_SIMPLEX, 2,  cv::Scalar(250,0,0), 5, 5, false);
						cv::putText(rgbMat, "as possible !", Point(70,300), FONT_HERSHEY_SIMPLEX, 2,  cv::Scalar(250,0,0), 5, 5, false);
				...
				if (x_avg < x_circle[k]+30 && x_avg > x_circle[k]-30 && y_avg > y_circle[k]-30 && y_avg < y_circle[k]+30 && old_minVal-minVal > 1 && !Playing_finished) 
				{
					if (!Playing) {
						Playing = TRUE;
						Arrived = FALSE;
			...
			if (timer == 0 && !Nobody) {
				...
				if (idle !=0 && Playing) {
					...
					if (points_game >= 7 && points_game < 13) {
						...
						cv::putText(rgbMat, "Humm... ", Point(30,200), FONT_HERSHEY_SCRIPT_SIMPLEX, 2,  cv::Scalar(250,0,0), 3, 3, false);
						cv::putText(rgbMat, "not great", Point(60,280), FONT_HERSHEY_SCRIPT_SIMPLEX, 2,  cv::Scalar(250,0,0), 3, 3, false);
						cv::putText(rgbMat, "You need a drink !", Point(10,360), FONT_HERSHEY_SCRIPT_SIMPLEX, 2,  cv::Scalar(250,0,0), 3, 3, false);
					} else if (points_game >= 13) {
						cv::putText(rgbMat, "AMAZING !!!", Point(110,200), FONT_HERSHEY_SCRIPT_SIMPLEX, 2,  cv::Scalar(250,0,0), 5, 5, false);
		 ...
		 *
		 *
		 *  FLIP THE IMAGE : flippedX = Width - currentX and use flippedX rather than currentX
		 *
		 * OR : cvconvertimage : http://www710.univ-lyon1.fr/~bouakaz/OpenCV-0.9.5/docs/ref/OpenCVRef_Highgui.htm
		 *
		 */
